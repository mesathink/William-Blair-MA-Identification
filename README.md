...
The dataset gets validated against a fixed 24 column schema on
...
### 2. Target profile form

The user picks a sector and a deal size (the two required
...
### 3. Tier 1 and Tier 2 matching, deterministic

Tier 1 pulls every acquirer with a
...
often collapse to the same pair regardless of the target, since those firms dominate the overlap count everywhere.
This is a limitation for sure, and given more time, something that would be revised and tested.

### 4. Tier 3 matching, agentic

If Tier 1 and 2 combined still haven't found 10
...
Every candidate acquirer gets four independent
...
### 6. Candidate ranking, deterministic

Candidates are stack ranked by tier first,
...
### 7. Report generation, agentic

`POST /api/reports` runs one agent conversation
...
`build_report_docx()` builds a formatted `.docx` straight
...
Backend is FastAPI, frontend is plain HTML/CSS/JS served as
...
The active dataset and the last completed report run both live
...
The CSV is read once at startup, or replaced on upload, into a
...
The Tier 2/3 evaluator agent mentioned above
...
### The scoring is skewed toward a specific set of fields

Results are skewed towards sector and deal size,
...
### "Regional" for geography is an assumption, not a value in the data
...
### The plus or minus 20% deal size tolerance (40% for Tier 3's wider search)
...
### Sector adjacency is data driven, not a hardcoded table

Which sector counts as
